...
from llamaclient import generate_headlines_from_news_data  # Import the Llama client function

# Function to handle weather, news, and map-based tasks based on voice commands
def handle_voice_command(command):
    ...
    if task == "weather":
        if location:
            ...
            weather_data = fetch_weather_data(location).get("weather")
            if weather_data:
        ...
        if location:
            if topic:
                ...
                news_data = fetch_news_datatopic(location, topic)  # Call the fetch function
            else:
                print(f"Fetching top news for '{location}'...")
                news_data = fetch_news_data(location)  # Fallback if no topic is provided

            if news_data:
                print(f"News data fetched for {location}. Processing headlines...")

                # Pass news articles to Llama client for headline generation
                headlines = generate_headlines_from_news_data(news_data)  # Pass the list of articles
                print("Generated Headlines:")
                for idx, headline in enumerate(headlines, 1):
                    print(f"{idx}. {headline}")  # Print each headline
