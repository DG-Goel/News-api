...
SEARCH_NEWS_API_URL = "https://api.worldnewsapi.com/top-news"
...
def split_text_into_chunks(text, max_tokens):
    """
    Split long text into smaller chunks that the PEGASUS model can handle.
    """
...
def generate_headlines_from_news_data(news_data):
    """
    Generate concise headlines from the provided news data.
    """
...
def query_pegasus_summarizer(text):
    """
    Generate a summary (headline) using the PEGASUS model.
    """
    try:
...
def process_in_chunks(prompt, max_tokens=MAX_TOKENS_PER_CHUNK):
    """
    Process the prompt in chunks if it exceeds the model's token limit.
    """
...
def fetch_news_data(location):
    """
    Fetch news from World News API based on location, then generate headlines using PEGASUS.
    """
